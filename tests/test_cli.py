...
def test_parser_accepts_dry_run() -> None:
    args = _build_parser().parse_args([*_BASE, "--dry-run"])
...
def test_parser_name_arg() -> None:
    args = _build_parser().parse_args([*_BASE, "--name", "custom"])
...
def test_parser_flags_splits_into_tuple() -> None:
    args = _build_parser().parse_args(
        [*_BASE, "--flags", "--model sonnet-4 --max-turns 5"]
    )
...
def test_parser_scenario_accepts_multiple() -> None:
    args = _build_parser().parse_args([*_BASE, "--scenario", "a", "b"])
...
def test_parser_output_parses_as_path() -> None:
    args = _build_parser().parse_args([*_BASE, "--output", "/tmp/out"])
...
def test_parser_env_single() -> None:
    args = _build_parser().parse_args([*_BASE, "-e", "FOO=bar"])
    assert args.env == ["FOO=bar"]


def test_parser_env_multiple() -> None:
    args = _build_parser().parse_args([*_BASE, "-e", "A=1", "-e", "B=2"])
    assert args.env == ["A=1", "B=2"]


def test_parser_env_long_form() -> None:
    args = _build_parser().parse_args([*_BASE, "--env", "X=y"])
    assert args.env == ["X=y"]
